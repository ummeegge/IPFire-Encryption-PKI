...
# Purpose: Core GPG operations – now with centralized logging
# Version: 0.4.1 – Encryption::Logging integration
#===============================================================================
...
# === Logging Helpers ===
sub log_debug { &Encryption::Logging::log_message($MODULE, 3, @_) if ($CONFIG{'DEBUG'} // '') eq 'on'; }
sub log_info  { &Encryption::Logging::log_message($MODULE, 2, @_); }
sub log_warn  { &Encryption::Logging::log_message($MODULE, 1, @_); }
sub log_error { &Encryption::Logging::log_message($MODULE, 0, @_); }

#=====================================================================
# Extract email from GPG UID string
#=====================================================================
sub extract_email {
	my ($uid) = @_;
...
sub load_config {
	...
	unless (-d $CONFIG{'GPGDIR'}) {
		...
		log_info("Created GPGDIR: $CONFIG{'GPGDIR'}");
	}
	return 1;
}
...
sub ensure_gpg_infrastructure {
	...
	unless (glob("$CONFIG{'GPGDIR'}/pubring.*") || glob("$CONFIG{'GPGDIR'}/secring.*")) {
		...
		log_info("Initialized empty GPG keyring in $CONFIG{'GPGDIR'}");
	}
...
sub list_keys {
	...
	log_debug("Listed " . scalar(@keys) . " keys");
	return @keys;
}
...
sub import_key {
	...
	unless ($exact_match) {
		...
		my $msg = $partial
			? "Partial email match for '$recipient_clean'"
			: "No match for '$recipient_clean'";
		log_warn("$msg. Key imported: $fingerprint");
		return "Warning: $msg. Key imported: $fingerprint";
	}
...
>/dev/null 2>&1
EOF
	log_info("Imported key $fingerprint for $recipient");
	return $fingerprint;
}
...
sub handle_upload_and_import {
	...
	if ($result && $result !~ /^(No|GPG|ECC)/) {
		if ($result eq 'unchanged') {
			log_info("Upload: key unchanged");
			return (1, 'unchanged', "Key already exists (unchanged)");
		} else {
			log_info("Upload: key $result imported");
...
sub save_settings {
	...
	if ($needs_write) {
		...
		if (&General::writehash($CONFIG_FILE, \%new_config)) {
			...
			$changed = 1;
			log_info("Settings saved: GPG_KEY=$new_gpg_key, DEBUG=$new_debug");
		} else {
			push @messages, "Failed to write config file.";
			log_error("Failed to write $CONFIG_FILE");
		}
	} else {
...
sub run_test_and_render {
	...
	log_info("Test encryption successful for $fp");
